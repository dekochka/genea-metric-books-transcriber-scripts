# GeneaScript - Інструмент для транскрипції метричних книг

| <img src="img/GeneaScript_logo_med.png" alt="GeneaScript" width="120"/> | Цей інструмент автоматично розпізнає та переписує рукописні записи з метричних книг (акти народження, смерті та шлюбів) зі сканованих зображень, використовуючи штучний інтелект Google Gemini. Інструмент підтримує **два режими роботи**:<br/><br/>• **LOCAL Mode** (Рекомендовано): Обробка зображень з локальної папки, використовуючи Gemini Developer API (без налаштування Google Cloud)<br/>• **GOOGLECLOUD Mode** (Розширений): Обробка зображень з Google Drive, використовуючи Vertex AI та створення документів Google Docs |
|:---:|:---|
...
> **⚠️ ВАЖЛИВО:** Штучний інтелект допускає багато неточностей у перекладі імен та прізвищ - використовувати як приблизний переклад рукописного тексту та перевіряти з джерелом! За оцінками найточніша модель інтерпретації рукописного тексту Gemini 3.0 Flash (Gemini 3.0 Pro).
...
1. **Markdown файл** (`.md`): Структурований markdown з відформатованими транскрипціями
   - Приклад: [`ф487оп1спр526_20260126_081844.md`](data_samples/test_input_sample/ф487оп1спр526_20260126_081844.md)
...
2. **Word документ** (`.docx`): Документ Microsoft Word з відформатованими транскрипціями
   - Приклад: [`ф487оп1спр526_20260126_081844.docx`](data_samples/test_input_sample/ф487оп1спр526_20260126_081844.docx)
   - Огляд сесії з метаданими
   - Відформатовані транскрипції з жирним текстом для виділених термінів
   - Заголовки сторінок з номерами (наприклад, ф487оп1спр526-стр1)
...
**Новачок у Python?** Виконайте ці кроки для початку роботи на Mac:

1. **Завантажте проект:**
   - Перейдіть на https://github.com/dekochka/genea-metric-books-transcriber-scripts
   - Натисніть "Code" → "Download ZIP"
   - Розпакуйте ZIP-архів у зручну папку (наприклад, у Документи: `Документи/genea-transcriber`)

2. **Встановіть Python:**
   - Див. [Інструкції з встановлення Python](#крок-1-встановлення-python) нижче для детальної настройки macOS
   - Якщо встановлено Homebrew: відкрийте Термінал (Програми → Утиліти → Terminal) і введіть: `brew install python3`
   - Або завантажте Python 3.10+ з https://www.python.org/downloads/macos/

3. **Налаштуйте проект:**
   - Відкрийте Термінал (Програми → Утиліти → Terminal)
   - Перейдіть у папку проекту: `cd ~/Documents/genea-transcriber` (або ваш шлях до розпакованої папки)
   - Створіть середовище: `python3 -m venv venv`
   - Активуйте його: `source venv/bin/activate`
   - Встановіть залежності: `pip install -r requirements.txt`
...
5. **Запустіть майстра:**
   - У Терміналі (у папці проекту, з активованим середовищем): `python3 transcribe.py`
   - Слідуйте інтерактивним підказкам
...
3. **Word документ** (зберігається в папці з зображеннями):
   - `{archive_index}_YYYYMMDD_HHMMSS.docx` - Документ Microsoft Word з:
     ...
     - Заголовками сторінок з номерами (наприклад, ф487оп1спр526-стр1)
     - Приклад: [`ф487оп1спр526_20260126_081844.docx`](data_samples/test_input_sample/ф487оп1спр526_20260126_081844.docx)
